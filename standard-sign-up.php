...
//Display current date to input to database
$date = date('Y-m-d');
...
/* Adding users to the database */
		...
		if(!empty($result)) {
			...
			session_start();
			header("Location: standardpay.php");
			unset($_POST);
		} else {
...
var oCOMScript1569581=document.createElement('script');oCOMScript1569581.src="https://commondatastorage.googleapis.com/comslider/target/users/1524681763x1e0f572bf8dadfd64b25b63b86044f96/comslider.js?timestamp=1524682019&ct="+Date.now();oCOMScript1569581.type='text/javascript';document.getElementsByTagName("head").item(0).appendChild(oCOMScript1569581);
  ...
  <div class="box">
  <h1><b><u>Standard Membership Registration</u></b></h1>
<h4><b><u>Sign up with Fitness Guru</u></b></h4>
<h7>Please fill the form to create an account</h7>
...
<form id="register" name="register" method="post" action="standard-sign-up.php">
...
<?php } ?>
  ...
  <div class="input"><input type="text" name="email"  value="<?php if(isset($_POST['email'])) echo $_POST['email']; ?>" tabindex="3" /></div>
  <h3><b><u>Username</u></b></h3>
  <div class="input"><input type="text" name="username"  value="<?php if(isset($_POST['username'])) echo $_POST['username']; ?>" tabindex="4" /></div>
  <div class="input"><input type="hidden" name="membership"  value="Standard" readonly/></div>
<div class="input"><input type="hidden" name="signupdate" value="<?php echo $date;?>" tabindex="5" /></textarea></div>
  <br />
  <h3>Kindly use a strong password as a combination of alphabet and numbers</h3>
